...
//      var result = Convert.jsonDecode(body);
  ...
  Future<dynamic> post(String uri, dynamic body, {Map<String, String> headers}) async {
    try {
      ...
      LogUtil.v('[baseUrl=$baseUrl][uri=$uri][body=$body][statusCode=$statusCode][response=$responseBody]');
      //print('[baseUrl=$baseUrl][uri=$uri][body=$body][statusCode=$statusCode][response=$responseBody]');
      return result;
    } on Exception catch (e) {
      print('[baseUrl=$baseUrl][uri=$uri]exception e=${e.toString()}');
      return '';
    }
  }
}
